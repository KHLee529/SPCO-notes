...
由此可以得到，從抽樣結果估計 $\lambda$ 可以用 $\hat \lambda = {1 \over \bar X}$ 進行估計。

## 評估最符合抽樣結果之機率模型

由前面的內容可以知道怎麼去估計機率模型當中的參數，但是在估計參數的同時，也要選到最適合的機率模型，估計才有意義。

選擇機率模型的方法，主要有兩種方法

- [適合度檢定 (Goodness-of-Fit Test)](https://en.wikipedia.org/wiki/Goodness_of_fit): 以理論計算評估適合度。
- 圖解法: 以看圖 (Q-Q plot / P-P plot) 方式評估。

由於圖解法極度方便使用，此處僅先介紹圖解法。

### Q-Q Plot & P-P Plot

圖解法最原始的想法是，「假設一個現象遵守特定機率分佈，抽樣結果也應該有該機率分佈的趨勢」。
而 Q-Q Plot 與 P-P Plot 便是判斷抽樣結果與機率分佈的相似度的方式。

#### 百分位數 (Percentile) 與樣本百分位數 (Sample Percentile)

當一個值 $\eta(p)$ 在母體當中大於全體 $p\%$ 的資料時，我們稱 $p$ 為 $\eta(p)$ 的百分位數 (基本上就是 PR 值的概念)。
用數學方式描述如下

$$
F(\eta(p)) = p \Rightarrow \eta(p) = F^{-1} (p)
$$

而樣本百分位數也是一樣的概念，但是要特別注意的事，由於假設樣本當中勢必不包含最大值 ($p = 100$) 與最小值 ($p = 0$)，在計算樣本百分位數時，會將其排序扣掉 0.5 再計算百分位數。
意即，對於 $n$ 個樣本當中第 $i$ 個樣本的百分位數 $p_i$ 為

$$
p_i = {i - 0.5 \over n} \times 100 \%
$$

#### Q-Q Plot

在了解百分位數的概念後，Q-Q Plot 即為將抽樣中每一個樣本對應到機率模型當中相同百分位數的資料值，並作圖比較這兩個值的相似性。
通常以「機率模型中第 ${i - 0.5 \over n} \times 100 \%$ 的資料數值」作為 $x$ 軸，「抽樣結果中第 $i$ 筆資料」作為 $y$ 軸作散佈圖，若圖形越趨近 $x=y$ 直線，則推論該機率模型越好。

下圖為 Q-Q 圖的範例

![qq.png](https://github.com/KHLee529/SPCO-notes/blob/main/pics/qq.png?raw=true)

透過比較不同機率分佈估計所產生的 Q-Q 圖可以輕易的比較出何種機率分佈的相似度較高。

#### P-P Plot

上述的 Q-Q 圖是對抽樣結果與機率模型當中同一個位置的數值相似性進行比較，而 P-P 圖則是以抽樣與機率模型當中同一個數值的百分位數進行比較。
也就是說， P-P 圖是將「抽樣結果中第 $i$ 筆資料在資料模型中的百分位數」作為 $x$ 軸，「抽樣結果中第 $i$ 筆資料在抽樣結果中的樣本百分位數」為 $y$ 軸作散佈圖。
與 Q-Q 圖相同，若圖形越接近 $x=y$ 直線，則推測該機率模型越好。\
(注意：由於比較資料為百分位數，故 $x$ 與 $y$ 軸範圍皆應落在 $[0,1]$)

下圖為 P-P 圖範例

![pp.png](https://github.com/KHLee529/SPCO-notes/blob/main/pics/pp.png?raw=true)
